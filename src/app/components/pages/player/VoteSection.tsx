...
export const VoteSection = React.memo(forwardRef<VoteSectionHandle, VoteSectionProps>(function VoteSection({
  ...
  return (
    <>
      {/* ── Maire election: glassmorphic info card ── */}
      {isMaireElection && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4 }}
          className="rounded-2xl text-center overflow-hidden relative mb-4"
          style={{
            background: 'rgba(0,0,0,0.3)',
            border: '1px solid rgba(255,255,255,0.1)',
            boxShadow: '0 4px 4px rgba(0,0,0,0.25)',
            padding: 'clamp(14px, 3vh, 20px) clamp(12px, 4vw, 20px)',
          }}
        >
          <div className="flex flex-col items-center gap-2">
            <span style={{ fontSize: 'clamp(32px, 10vw, 50px)', lineHeight: 1 }}>🏛️</span>
            <h2 style={{
            ...
            <p style={{
              ...
              Votez pour elire le Maire du village. Son vote comptera double !
            </p>

            {/* Candidacy button (inside info card) */}
            {currentPlayerId !== null && currentPlayerAlive && (
              <div className="mt-3">
                {isCandidate ? (
                  <motion.button
                    whileTap={{ scale: 0.96 }}
                  ...
                  >
                    ...
                    <span style={{ fontFamily: '"Cinzel", serif', fontSize: '0.7rem', fontWeight: 700 }}>
                      Vous êtes candidat(e)
                    </span>
                    <span style={{ color: 'rgba(212,168,67,0.5)', fontSize: '0.55rem', fontFamily: '"Cinzel", serif' }}>
                      · Retirer
                    </span>
                  </motion.button>
                ) : (
                  <motion.button
                    ...
                    style={{
                      ...
                      boxShadow: '0 3px 12px rgba(212,168,67,0.35)',
                      color: '#0a0e1a',
                    }}
                  >
                    <Crown size={13} style={{ color: '#0a0e1a' }} />
                    <span style={{ fontFamily: '"Cinzel", serif', fontSize: '0.7rem', fontWeight: 700 }}>
