        ...
        <div className="relative flex flex-wrap md:flex-nowrap items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              {state.poisonedPlayers?.[selectedPlayerData.id] && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full" style={{ background: 'rgba(101,163,13,0.1)', border: '1px solid rgba(101,163,13,0.25)', color: '#65a30d', fontSize: '0.55rem', fontFamily: '"Cinzel", serif' }}>🧪 Empoisonné</span>
              )}
      ...
      {isNight && state.nightStep === 'active' && selectedPlayerData.alive && (
        <div className="rounded-xl p-4 mb-5" style={{ background: 'rgba(124,141,181,0.04)', border: '1px solid rgba(124,141,181,0.12)' }}>
          ...
          <div className="space-y-2 mt-3">
            ...
            {selectedPlayerData.role === 'oracle' && (() => { const oracleUsed = !!(state.oracleUsed || {})[selectedPlayerData.id]; return (<div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: oracleUsed ? 'rgba(107,142,90,0.06)' : 'rgba(124,58,237,0.04)', border: `1px solid ${oracleUsed ? 'rgba(107,142,90,0.15)' : 'rgba(124,58,237,0.1)'}` }}><span className="text-lg">🌙</span><div className="flex-1 min-w-0"><span style={{ fontFamily: '"Cinzel", serif', color: '#7c3aed', fontSize: '0.75rem' }}>Issue de la nuit</span><p className="truncate" style={{ color: t.textMuted, fontSize: '0.6rem', marginTop: '0.1rem' }}>{oracleUsed ? 'A consulté les étoiles' : 'En attente'}</p></div>{oracleUsed ? (<div className="w-6 h-6 rounded-full flex items-center justify-center" style={{ background: 'rgba(107,142,90,0.15)' }}><Check size={12} style={{ color: '#6b8e5a' }} /></div>) : (<div className="w-6 h-6 rounded-full flex items-center justify-center" style={{ background: `rgba(${t.overlayChannel}, 0.04)` }}><Clock size={12} style={{ color: t.textMuted }} /></div>)}</div>); })()}
