...
export function useGameActions({ setState, stateRef, localModeRef }: ActionsDeps) {
  ...
  const setDiscoveryWolfTarget = useCallback((wolfId: number, targetId: number) => {
    setState((s) => ({
      ...s,
      discoveryWolfTargets: { ...(s.discoveryWolfTargets ?? {}), [wolfId]: targetId },
    }));
  }, [setState]);
